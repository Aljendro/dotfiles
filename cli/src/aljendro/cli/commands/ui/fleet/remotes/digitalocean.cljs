  ...
  (start! [_this]
    (let [droplet-id (atom nil)
      ...
      (-> (shell/exec!
          ...
          (.then #(shell/exec! (str "ssh " remote-name " " (js/JSON.stringify "cloud-init status --wait")
                                    " && doctl compute droplet-action reboot " @droplet-id " --wait")
                               {:retries  1
                                :delay-ms 1000})))))
  (provision! [_this]
    (-> (shell/exec! (str "ssh " remote-name " " remotes-common/dotfiles-install-command-str))
        (.then #(shell/exec! (str "doctl compute droplet get " remote-name " --format ID --no-header")))
        (.then #(shell/exec! (str "doctl compute droplet-action reboot " (.trim %) " --wait")))))
  (enter! [_this]
