...
(defrecord LimaRemote [id branch remote-type remote-name status last-sync]
  ...
  (start! [_this]
    (let [ca-certs (.. js/process -env -FLEET_LIMA_CA_CERTS)
      ...
      (shell/exec! (str "limactl start --name='" remote-name "' --mount-none"
                                 ...
                                 (map (fn [p]
                                        (let [rel (.replace p (str home "/") "")]
                                          (str "ssh " ssh-host
                                               ...
                                               " " ssh-host ":~/" rel))))
                                 (str/join " && ")))]
      (shell/exec! (str (when scp-cmds (str scp-cmds " && "))
                        "ssh " ssh-host
  ...
  (delete! [_this]
    (-> (shell/exec! (str "limactl stop " remote-name " 2>/dev/null || true"))
        (.then #(shell/exec! (str "limactl delete " remote-name " 2>/dev/null || true")))))
  (rsync-to! [_this]
